...
public class BookShop {
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException {
		...
		while (true) {
			System.out.println(
					"¬ведите 1 дл€ вывода всей литературы, 2 дл€ добавлени€ новой, 3 дл€ записи в файл, 4 дл€ выхода из программы");
			...
			if (i == 2) {
				...
				if (j == 1) {
					...
					String sql = "INSERT INTO bookshop.literature(author, title, publisher, year, type) " + "VALUES (\'"
							+ author + "\', \'" + title + "\', \'" + publisher + "\', \'" + year + "\', \'" + type
							+ "\');";
				...
				if (j == 2) {
					...
					String sql = "INSERT INTO bookshop.literature(publisher, title, year, numbers_per_year, type) "
							+ "VALUES (\'" + publisher + "\', \'" + title + "\', \'" + year + "\', \'" + numbersPerYear
							+ "\', \'" + type + "\');";
					jdbcTemplate.update(sql);
				}
			}
			if (i == 3) {
				System.out.println("¬ведите 1 дл€ записи в XML, 2 дл€ записи в JSON");
				...
				if (k == 2) {
					...
					try {
						Writer writer = new FileWriter("literature.json");
						gson.toJson(booksAndMagazines, writer);
						writer.close();
					} catch (IOException e) {
						e.printStackTrace();
					}
